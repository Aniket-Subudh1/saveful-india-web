...
export default function NewIngredientPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        setLoadingData(true);
        const [categoriesRes, ingredientsRes, dietsRes, sponsorsRes, hacksRes, stickersRes] = await Promise.allSettled([
        ...
        setIngredients(ingredientsRes.status === "fulfilled" ? ingredientsRes.value : []);
        setDiets(dietsRes.status === "fulfilled" ? dietsRes.value : []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!form.averageWeight || Number(form.averageWeight) <= 0) throw new Error("Average weight must be greater than 0");
  ...
  return (
    <DashboardLayout config={sidebarConfig}>
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="font-saveful-bold text-3xl text-saveful-green">Create Ingredient</h1>
          <p className="font-saveful text-saveful-gray">Add a new ingredient with complete details</p>
        </div>
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  setForm({ ...form, averageWeight: val === '' || isNaN(num) ? 1 : num });
                }}
                required
                className="w-full rounded-lg border-2 border-gray-300 px-4 py-2.5 font-saveful focus:border-saveful-green focus:outline-none focus:ring-2 focus:ring-saveful-green/20"
              />
            ...
            <div>
              ...
              <div className="rounded-lg border-2 border-gray-200 bg-gray-50 p-3 flex flex-wrap gap-2 max-h-40 overflow-y-auto">
                {loadingData ? (
                  ...
                  diets.map((diet) => {
                    const checked = form.suitableDiets?.includes(diet._id || diet.id!);
                    return (
                      <label key={diet._id || diet.id} className={`flex items-center gap-2 rounded-lg border px-3 py-1.5 text-sm ${checked ? 'border-saveful-green bg-white' : 'border-gray-200 bg-white'}`}>
                        <input
                          type="checkbox"
                          checked={checked}
                          onChange={(e) => {
